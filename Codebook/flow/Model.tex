...
\begin{itemize}
    ...
    \begin{enumerate}
      \item For each $v \in V$ create a copy $v^\prime$, connected $u^\prime \to v^\prime$ with weight $w((u, v))$.
      \item Connect $v \to v^\prime$ with weight $2\mu(v)$, where $\mu(v)$ is the cost of the cheapest edge incident to $v$.
      \item The minimum weight perfect matching on the resulting graph is equivalent to the minimum weight edge cover of $G$ by changing each $(v, v^\prime)$ to the cheapest edge of $v$.
    \end{enumerate}
\end{itemize}
