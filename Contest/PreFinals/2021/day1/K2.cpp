...
int main() {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    for (int j = 0; j < h; ++j) {
      ...
      double oo = acos((d * d + dd * dd - hr * hr) / (2 * d * dd));
    ...
    for (int j = 0; j < n; ++j) if (i != j) {
      ...
      event.emplace_back(o - eps - eps, d + eps, -1, -j - 1);
      event.emplace_back(o + eps + eps, d + eps, -2, -j - 1);
    }
    ...
    sort(event.begin(), event.end(), [&] (const tuple<double, double, int, int> &a, const tuple<double, double, int, int> &b) -> bool {
      if (same(get<0>(a), get<0>(b))) {
        int at = get<2>(a);
        int bt = get<2>(b);
        if (bt == -1) return 0;
        if (at == -1) return 1;
