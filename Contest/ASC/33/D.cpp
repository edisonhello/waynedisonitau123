...
void dfs_term(int now) {
  ...
  for (auto s : rules[now]) {
    if (AllTerminals(s)) {
      ...
      for (char i : s) {
        if (!IsVariable[i]) go_nonempty[now] = 1;
        else {
          if (!vis[i]) dfs_term(i);
          if (go_nonempty[i]) go_nonempty[now] = 1;
        }
      }
    }
  }
...
void try_front(int now, int target, int non_empty) {
  ...
  for (string s : rules[now]) {
    ...
    if (ok) front_term[now] = 1;
  }
}

void try_back(int now, int target) {
  vis[now] = 1;
...
int main() {
  ...
  for (int i = 'A'; i <= 'Z'; ++i) if (front_term[i] || back_term[i]) {
    cout << "infinite" << endl;
    exit(0);
  }
